...
import { DeleteStudentInClassExamSchedule } from "../../../services/studentInExamScheduleService";

// eslint-disable-next-line react/prop-types
function PopUpRemoveStudentInExamScheduke({ studentExamId, onDeleted }) {
  ...
  const handleDeleteClick = async () => {
    ...
    setShowAlert(false);
    try {
      const response = await DeleteStudentInClassExamSchedule(studentExamId);
      if (response.isSuccess) {
        setShowAlert("success");
        setSuccessMessage(response.message);
        setTimeout(() => {
          setShowAlert(false);
          onDeleted(studentExamId);
        }, 1500);
        setIsFormVisible(false); 
      } else {
...
export default PopUpRemoveStudentInExamScheduke;
